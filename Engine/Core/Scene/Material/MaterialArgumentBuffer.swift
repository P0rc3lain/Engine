...
struct MaterialArgumentBuffer {
    ...
    func create() -> MTLBuffer? {
        ...
        guard let encoder = device.makeArgumentEncoder(arguments: [albedoTexture,
                                                                   roughnessTexture,
                                                                   normalsTexture,
                                                                   metallicTexture]) else {
            return nil
        }
        let requiredSize = encoder.encodedLength
        guard let argumentBuffer = device.makeBuffer(length: requiredSize,
