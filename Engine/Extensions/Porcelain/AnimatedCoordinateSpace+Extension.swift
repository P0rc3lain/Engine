...
extension AnimatedCoordinateSpace {
    ...
    static public var `static`: AnimatedCoordinateSpace {
        AnimatedCoordinateSpace(translation: PNAnySampleProvider(AnimatedFloat3.defaultTranslation),
                                rotation: PNAnySampleProvider(AnimatedQuatf.defaultOrientation),
                                scale: PNAnySampleProvider(AnimatedFloat3.defaultScale))
    }
    static public func `static`(from transformation: simd_float4x4) -> AnimatedCoordinateSpace {
        let decomposed = transformation.decomposed
        return AnimatedCoordinateSpace(translation: PNAnySampleProvider(AnimatedFloat3.static(from: decomposed.translation)),
